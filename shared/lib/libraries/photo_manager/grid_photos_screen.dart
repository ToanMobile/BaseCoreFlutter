...
class _GridPhotosScreenState extends State<GridPhotosScreen> with AutomaticKeepAliveClientMixin<GridPhotosScreen>, WidgetsBindingObserver {
  ...
  ThumbOption get thumbOption => const ThumbOption(width: 139, height: 139, quality: 86);
          ...
          return provider.itemsSelected.isNotEmpty
              ? InkWell(
                  child: Container(
                    ...
                    child: Text(
                      widget.isMultiple
                          ? "${'selected'.text} ${provider.itemsSelected.length}/${provider.totalPhoto} ${'images'.text}"
                          : 'complete'.text,
                      style: text16.bold.textWhiteColor,
                    ),
                  ),
