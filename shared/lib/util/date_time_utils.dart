...
import 'package:shared/res/theme/text_theme.dart';
...
String readTimeStampBySecond(int? timestamp) {
  ...
  if (diff.inSeconds <= 0) {
    ...
    time = diff.inMinutes.toString() + " " + 'minute'.text.toLowerCase();
  } else if (diff.inHours > 0 && diff.inHours < 24) {
    time = diff.inHours.toString() + " " + 'hour'.text.toLowerCase();
  } else if (diff.inDays > 0 && diff.inDays < 7) {
    time = diff.inDays.toString() + ' ' + 'day'.text.toLowerCase();
  } else {
    time = (diff.inDays / 7).floor().toString() + ' ' + 'week'.text.toLowerCase();
  }
